    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200 sticky top-8">
                ...
                <form action="<?= htmlspecialchars(app_url('contas-fixas')) ?>" method="POST" class="space-y-4">
                    <?= csrf_field() ?>
                    <div>
                        ...
                        <?php foreach($contas as $cf): ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                    <?php if(!$cf['pago']): ?>
                                        <form action="<?= htmlspecialchars(app_url('pagar-conta-fixa')) ?>" method="POST" class="inline-block">
                                            <?= csrf_field() ?>
                                            <input type="hidden" name="id" value="<?= (int) $cf['id'] ?>">
                                            <button class="bg-slate-900 text-white hover:bg-indigo-600 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all shadow-sm">Baixar Pago</button>
                                        </form>
                                    <?php endif; ?>
                                    
                                    <form action="<?= htmlspecialchars(app_url('deletar-conta-fixa')) ?>" method="POST" class="inline-block align-middle ml-2" onsubmit="return confirm('Deseja remover este molde de conta fixa?')">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="id" value="<?= (int) $cf['id'] ?>">
                                        <button class="text-slate-300 hover:text-rose-500 transition-colors" title="Remover Molde"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
