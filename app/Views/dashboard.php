                        ...
                        <?php foreach($transacoes as $tr): ?>
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                    ...
                                    <form action="<?= htmlspecialchars(app_url('deletar-transacao')) ?>" method="POST" class="inline-block" onsubmit="return confirm('Deseja realmente apagar esta conta? As dívidas dos amigos vinculados também serão removidas.')">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="id" value="<?= (int) $tr['id'] ?>">
                                        <button class="text-rose-400 hover:text-rose-700 transition-colors" title="Excluir">
